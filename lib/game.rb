    ...
    def turn
        ...
        while well_done < 1
            if (case_to_fill == "A1" && @board.a_one.state == " ")
                @board.a_one.state = @current_player.value.to_s
                well_done += 1 
            elsif case_to_fill == "A2" && @board.a_two.state == " "
                @board.a_two.state = @current_player.value.to_s
                well_done += 1
            elsif case_to_fill == "A3" && @board.a_three.state == " "
                @board.a_three.state = @current_player.value.to_s
                well_done += 1
            elsif case_to_fill == "B1" && @board.b_one.state == " "
                @board.b_one.state = @current_player.value.to_s                        
                well_done += 1            
            elsif case_to_fill == "B2" && @board.b_two.state == " "
                @board.b_two.state = @current_player.value.to_s
                well_done += 1
